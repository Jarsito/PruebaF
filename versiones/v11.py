...
PRODUCTOS = {
    instrumento: ProductoMusical(
        instrumento,
        [(marca, precio, 10) for marca, precio in MARCAS[instrumento].items()]
...
class CarritoItem:
    ...
    def calcular_precio_unitario(self):
        return next(precio for m, precio, _ in self.producto.marcas_precios_stock if m == self.marca)
...
def main():
    ...
    while True:
        ...
        instrumento_seleccionado = PRODUCTOS.get(producto_seleccionado)
        if instrumento_seleccionado:
            producto = PRODUCTOS[producto_seleccionado]
            producto.log()

            # Obtener todas las marcas disponibles
            marcas_disponibles = [marca for marca, _, _ in producto.marcas_precios_stock]
            print("Marcas Disponibles:", marcas_disponibles)

            marca_seleccionada = input("Seleccione una marca: ").lower()  # Utilizar minúsculas
            ...
            if marca_seleccionada in marcas_disponibles:
                ...
                marca_index = marcas_disponibles.index(marca_seleccionada)
                _, _, stock = producto.marcas_precios_stock[marca_index]

                if 0 < cantidad <= stock:
                    carrito.append(CarritoItem(producto, marca_seleccionada, cantidad))
                    producto.marcas_precios_stock[marca_index] = (marca_seleccionada, producto.marcas_precios_stock[marca_index][1], stock - cantidad)
    ...
    if carrito:
        ...
        for item in carrito:
            ...
            if item.marca not in total_por_marca:
                ...
                total_por_marca[item.marca] += precio_total
            print(f"{item.producto.nombre.capitalize()} - Marca: {item.marca.capitalize()} - Cantidad: {item.cantidad} - Precio unitario: ${precio_unitario:.2f} - Precio total: ${precio_total:.2f}")
